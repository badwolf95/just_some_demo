...
class PC{
	...
	public static function init_method(){
		//注意是isset，没有下划线
		self::$method = isset($_GET['method'])?doaddslashes($_GET['method']):'login';
	}
	//初始化controller值
	public static function init_Controller(){
		self::$controller = isset($_GET['controller'])?doaddslashes($_GET['controller']):'admin';
	}
	//统一执行以上各个配置
	//需要从配置文件传入所需配置数据的数组
