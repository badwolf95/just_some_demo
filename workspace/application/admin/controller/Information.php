...
class Information extends Common {
	...
	public function index(){
		...
		$information = model('Information')->whereTime('create_time',$time)->where($map)->order('create_time','desc')->paginate($pagesize);
		
		foreach($information as $infor){
			$read_user_num = explode(',',$infor->read_user_id);
			$infor->read_user_num = count($read_user_num)-1;
		}
	...
	public function add(){
		if(input('post.')){
			//验证各字段
			$rules = [
				['types','require|in:1,2,3','请选择通知对象|通知对象非法'],
				['members','requireIf:types,2|requireIf:types,3','请输入用户的五位ID|用输入组织的四位ID'],
				['content','require|length:30,900','请填写通知内容|内容范围在10到300个汉字之间']
			];
